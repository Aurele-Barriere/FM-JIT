...
Definition def_insert (r:reg) (adr:def_abs_dr) : def_abs_dr :=
  ...
  | DefFlatRegset.Bot => (* DefFlatRegset.Inj (PositiveSet.singleton r) *)
    DefFlatRegset.Bot   (* to comply with the new Kildall *)
...
Definition def_dr_transf (c:code) (l:label) (adr:def_abs_dr) : def_abs_dr :=
  ...
  | Some i =>
    ...
    | MemGet dstreg adreg next =>
      def_insert dstreg adr
    | _ =>                       (* other instructions don't declare registers *)
      adr
    end
  end.

Definition defined_regs_analysis (c:code) (params:list reg) (entry:label): option def_abs_state:=
  DS.fixpoint c IR.successors (def_dr_transf c) entry (def_entry_abs_dr params).
...
Proof.
  ...
  - destruct s2.
    ...
    + simpl in H. simpl. destruct (Bool.eqb b b0) eqn:EQ; inv H.
...
Proof.
  ...
  intros n. unfold def_dr_transf. destruct ((ver_code v)#n); try destruct i0; try apply DS.L.eq_refl.
Qed.
...
Proof.
  ...
  { eapply DS.fixpoint_entry; eauto. }
  eapply defined_increasing; eauto.
